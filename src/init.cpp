...
bool AppInit2(int argc, char* argv[])
{
    ...
    if (mapArgs.count("-?") || mapArgs.count("--help"))
    {
        string strUsage = string() +
          _("Peercoin version") + " " + FormatFullVersion() + "\n\n" +
          _("Usage:") + "\t\t\t\t\t\t\t\t\t\t\n" +
            "  peercoind [options]                   \t  " + "\n" +
    ...
    for (int i = 1; i < argc; i++)
        if (!IsSwitchChar(argv[i][0]) && !(strlen(argv[i]) >= 7 && strncasecmp(argv[i], "peercoin:", 7) == 0))
            fCommandLine = true;
    ...
    if (!lock.try_lock())
    {
        ThreadSafeMessageBox(strprintf(_("Cannot obtain a lock on data directory %s.  Peercoin is probably already running."), GetDataDir().string().c_str()), _("Peercoin"), wxOK|wxMODAL);
        return false;
    }
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            ...
            ThreadSafeMessageBox(strErrors.str(), _("Peercoin"), wxOK | wxICON_ERROR | wxMODAL);
            return false;
        }
        else
    ...
    if (mapArgs.count("-proxy"))
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid -proxy address"), _("Peerccoin"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (!fNoListen)
    {
        ...
        {
            ThreadSafeMessageBox(strError, _("Peercoin"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (mapArgs.count("-paytxfee"))
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid amount for -paytxfee=<amount>"), _("Peercoin"), wxOK | wxMODAL);
            return false;
        }
        if (nTransactionFee > 0.25 * COIN)
            ThreadSafeMessageBox(_("Warning: -paytxfee is set very high.  This is the transaction fee you will pay if you send a transaction."), _("Peercoin"), wxOK | wxICON_EXCLAMATION | wxMODAL);
    }
    ...
    if (mapArgs.count("-reservebalance")) // peercoin: reserve balance amount
    {
        ...
        {
            ThreadSafeMessageBox(_("Invalid amount for -reservebalance=<amount>"), _("Peercoin"), wxOK | wxMODAL);
            return false;
        }
    }
    ...
    if (!CreateThread(StartNode, NULL))
        ThreadSafeMessageBox(_("Error: CreateThread(StartNode) failed"), _("Peercoin"), wxOK | wxMODAL);
